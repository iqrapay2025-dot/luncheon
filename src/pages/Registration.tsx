...
const inputStyle: React.CSSProperties = {
  ...
  padding: "11px 14px",
  borderRadius: 10,
  border: "1.5px solid rgba(61,21,80,0.15)",
  fontSize: 14,
  color: "#1A1A2E",
  background: "#ffffff",
  outline: "none",
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: "11px 16px",
        borderRadius: 10,
        border: `1.5px solid ${active ? "#3D1550" : "rgba(61,21,80,0.12)"}`,
        background: active ? "rgba(61,21,80,0.06)" : "#ffffff",
...
function LeftPanel({
  ...
  const pkgData = pkg ? getPackageByName(pkg) : undefined
    ...
    >
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            "linear-gradient(180deg, rgba(26,10,48,0.28) 0%, rgba(26,10,48,0.15) 40%, rgba(26,10,48,0.85) 100%)",
        }}
      />
      <div
        style={{
      ...
      >
        ...
        {pkg && (
          ...
          >
            {pkgData ? `${pkg} · ${formatPrice(pkgData.price)}` : pkg}
          </div>
        )}
      </div>
        ...
        {PACKAGES.map((p) => {
          ...
          return (
            <button
              key={p.name}
              onClick={() =>
                setData({ package: p.name, amount: String(p.price) })
              }
              style={{
                width: "100%",
                display: "flex",
                ...
                borderRadius: 12,
                border: `1.5px solid ${
                  sel ? "#3D1550" : "rgba(61,21,80,0.12)"
                }`,
                background: sel ? "rgba(61,21,80,0.05)" : "#ffffff",
                cursor: "pointer",
                textAlign: "left",
                transition: "all 0.15s",
                boxShadow: sel ? "0 0 0 3px rgba(61,21,80,0.08)" : "none",
                minHeight: 64,
              }}
            >
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  border: `1.5px solid ${
                    sel ? "#3D1550" : "rgba(61,21,80,0.2)"
                  }`,
                  background: sel ? "#3D1550" : "transparent",
                  display: "flex",
                  alignItems: "center",
  ...
  return (
    <div>
      ...
      <FieldSet label="Dietary Notes (optional)">
        <textarea
          value={data.mealNote}
          onChange={(e) => setData({ mealNote: e.target.value })}
...
function Step3({
  ...
  const inp = (key: keyof FormData, ph: string, type = "text") => (
    <input
      ...
      onChange={(e) => setData({ [key]: e.target.value } as Partial<FormData>)}
      placeholder={ph}
      style={inputStyle}
      onFocus={(e) => (e.currentTarget.style.borderColor = "#3D1550")}
      onBlur={(e) =>
        (e.currentTarget.style.borderColor = "rgba(61,21,80,0.15)")
      }
    />
  )
  ...
  return (
    <div>
      ...
      <FieldSet
        label="Email Address"
        helper="Optional — required if you want your ticket emailed to you"
      >
        {inp("email", "you@example.com", "email")}
      </FieldSet>
      <FieldSet label="WhatsApp Number">
        {inp("whatsapp", "08012345678", "tel")}
      </FieldSet>
      <div
        className="reg-form-grid-2"
        style={{
      ...
      >
        <FieldSet label="Gender">
          {selField("gender", ["Male", "Female"])}
        </FieldSet>
        <FieldSet label="Level / Status">
          {selField("level", LEVEL_OPTS)}
        </FieldSet>
      </div>
      ...
      {data.level && (
        <FieldSet label={`Matric Number${needsMatric ? "" : " (optional)"}`}>
          <input
            ...
            onChange={(e) =>
              !isHaflah && setData({ matricNumber: e.target.value })
            }
            placeholder={isHaflah ? "" : "e.g. 18/52HA010"}
            disabled={isHaflah}
            style={{
              ...inputStyle,
              background: isHaflah ? "#F3F4F6" : "#ffffff",
              color: isHaflah ? "#9CA3AF" : "#1A1A2E",
              cursor: isHaflah ? "not-allowed" : "text",
            }}
            onFocus={(e) =>
              !isHaflah && (e.currentTarget.style.borderColor = "#3D1550")
            }
            onBlur={(e) =>
              (e.currentTarget.style.borderColor = "rgba(61,21,80,0.15)")
            }
          ...
          {isHaflah && (
            ...
            >
              Not applicable for Haflah Graduates
            </div>
          )}
        </FieldSet>
      )}
      <FieldSet label="Department">
        {inp("department", "Computer Science")}
      </FieldSet>
  ...
  return (
    <div>
      ...
      <FieldSet label="Would you like to customise your VIP souvenir?">
        <div style={{ display: "flex", gap: 10 }}>
          <ToggleBtn
            label="Yes, customise it"
            active={data.ikramWantsCustom === "Yes"}
            onClick={() => setData({ ikramWantsCustom: "Yes" })}
          />
          <ToggleBtn
            label="No, use default"
            active={data.ikramWantsCustom === "No"}
            onClick={() =>
              setData({ ikramWantsCustom: "No", ikramCustomization: "" })
            }
          />
        </div>
      </FieldSet>
      ...
      {data.ikramWantsCustom === "Yes" && (
        <FieldSet
          ...
          helper="Include preferred name, colour, and any special requests"
        >
          <textarea
  ...
  return (
    <div>
      ...
      <div style={sectionSub}>
        We are officially launching the alumni network at this event
      </div>
      <FieldSet label="Interested in joining MSSN Alumni Group 1.0?">
        <div style={{ display: "flex", gap: 10 }}>
          <ToggleBtn
            label="Yes, add me"
            active={data.joinAlumni === "Yes, add me"}
            onClick={() => setData({ joinAlumni: "Yes, add me" })}
          />
          <ToggleBtn
            label="No, skip"
            active={data.joinAlumni === "No, skip"}
            onClick={() => setData({ joinAlumni: "No, skip" })}
          />
        </div>
      </FieldSet>
        ...
        <>
          <FieldSet label="Full Name for Alumni Register">
            <input
              ...
              onChange={(e) => setData({ alumniName: e.target.value })}
          ...
          >
            <FieldSet label="Graduation Year">
              <input
                type="text"
                value={data.alumniYear}
                onChange={(e) => setData({ alumniYear: e.target.value })}
...
function Step6({
  ...
  return (
    <div>
      ...
      <div style={sectionSub}>
        Help us acquire a dedicated bus for student welfare
      </div>
      <div
        style={{
          background: "rgba(61,21,80,0.04)",
      ...
      >
        ...
        >
          A dedicated MSSN bus will ease transportation for programs, dawah
          activities, and inter-campus events.
        </p>
      </div>
      <FieldSet label="Would you like to pledge?">
        <div style={{ display: "flex", gap: 10 }}>
          <ToggleBtn
            label="Yes, I'll pledge"
            active={data.busDonate === "Yes, I'll pledge"}
            onClick={() => setData({ busDonate: "Yes, I'll pledge" })}
          />
          <ToggleBtn
            label="No, skip"
            active={data.busDonate === "No, skip"}
            onClick={() => setData({ busDonate: "No, skip" })}
          />
        </div>
      </FieldSet>
      {data.busDonate === "Yes, I'll pledge" && (
        <FieldSet label="Pledge Amount (₦)">
          <input
            type="number"
            value={data.busAmount}
            onChange={(e) => setData({ busAmount: e.target.value })}
...
function Step7({
  ...
  return (
    <div>
      ...
      <div style={sectionSub}>
        Transfer the amount below and upload your receipt to complete
        registration
      </div>
      {/* Amount to Pay — auto-populated from the selected package price */}
      <div
        style={{
          background: "#FFF8F0",
      ...
      <div
        style={{
          background: "#F0F4FF",
          border: "1px solid rgba(61,21,80,0.12)",
          borderRadius: 12,
          padding: "16px 18px",
          marginBottom: 20,
        }}
      >
        ...
        >
          ...
          <div>
            ...
            <strong>Zakariyah Habeeb-llahi Mukadam</strong>
          </div>
        </div>
      </div>
      <FieldSet label="Payer Name (as on receipt)">
        <input
          ...
          onChange={(e) => setData({ payerName: e.target.value })}
          placeholder="Name used during transfer"
          style={inputStyle}
          onFocus={(e) => (e.currentTarget.style.borderColor = "#3D1550")}
          onBlur={(e) =>
            (e.currentTarget.style.borderColor = "rgba(61,21,80,0.15)")
      ...
      <FieldSet label="Upload Payment Receipt">
        <label
          style={{
            display: "block",
            border: "1.5px dashed rgba(61,21,80,0.2)",
            borderRadius: 10,
            padding: "20px",
            textAlign: "center",
            cursor: "pointer",
            background: data.receiptFile ? "rgba(61,21,80,0.04)" : "#FAFAFA",
    ...
    val ? (
      <div
        ...
        style={{
          ...
          justifyContent: "space-between",
          padding: "9px 0",
          borderBottom: "1px solid rgba(61,21,80,0.07)",
          gap: 12,
        }}
      >
        <span
    ...
    <div>
      ...
      <div
        style={{
          ...
          padding: "16px 18px",
          marginBottom: 20,
          border: "1px solid rgba(61,21,80,0.08)",
        }}
      >
        {row("Package", data.package)}
        {row("Amount", pkgDisplay ? formatPrice(pkgDisplay.price) : "—")}
        {row("Meal", data.meal)}
        ...
        {data.joinAlumni === "Yes, add me"
          ...
          : null}
        {data.joinAlumni === "Yes, add me"
          ? row("Graduation Year", data.alumniYear)
          : null}
        {row(
          "Bus Pledge",
          data.busDonate === "Yes, I'll pledge"
      ...
      >
        ...
        >
          I confirm these details are accurate. I understand that{" "}
          <strong>no changes can be made after submission</strong> and
          allocation depends on payment verification.
        </span>
      </label>
    </div>
  )
...
export default function RegistrationPage({
  ...
  const setData = (patch: Partial<FormData>) =>
    setDataRaw((prev) => ({ ...prev, ...patch }))

  const selectedPkg = data.package ? getPackageByName(data.package) : undefined

  const activeSections = selectedPkg?.hasCustomization
    ? [0, 1, 2, 3, 4, 5, 6, 7]
    : [0, 1, 2, 4, 5, 6, 7]
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        amount: data.amount || String(selectedPkg?.price ?? 0),
        meal: data.meal,
        ikramCustomization: selectedPkg?.hasCustomization
          ? data.ikramWantsCustom === "Yes"
            ? data.ikramCustomization
            : "Default — no customisation"
          : "",
        ...
        >
              ...
              return (
                <div
                  ...
                  style={{
                    display: "flex",
                    alignItems: "center",
                    flex: i < activeSections.length - 1 ? 1 : 0,
                    minWidth: 0,
                  }}
                >
                  <div
                    ...
                    {done ? (
                      ...
                      <span
                        style={{
                          fontSize: 10,
                          fontWeight: 700,
                          color: active ? "#FDF8F0" : "rgba(61,21,80,0.35)",
                          fontFamily: "Outfit, sans-serif",
                        }}
                      >
                        {i + 1}
          ...
          <div
            style={{
              ...
              boxShadow: "0 2px 12px rgba(61,21,80,0.07)",
              border: "1px solid rgba(61,21,80,0.06)",
              marginBottom: 16,
            }}
          >
            {renderStep()}
          ...
          {submitError && (
            <div
              style={{
                ...
                border: "1.5px solid rgba(166,61,64,0.25)",
                borderRadius: 10,
                padding: "12px 16px",
                display: "flex",
                alignItems: "flex-start",
            ...
            >
              ...
              >
                ...
                <path
                  ...
                  strokeLinecap="round"
                />
                <circle cx="8" cy="11" r="0.75" fill="#A63D40" />
              </svg>
              <span
                style={{
                  fontFamily: "Manrope, sans-serif",
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <button
              ...
              onMouseOver={(e) =>
                (e.currentTarget.style.background = "rgba(61,21,80,0.05)")
              }
              onMouseOut={(e) =>
            ...
            {!isLast ? (
              <button
                ...
                style={{
                  ...
                  borderRadius: 10,
                  border: "none",
                  background: canGoNext() ? "#3D1550" : "#D1D5DB",
                  color: canGoNext() ? "#FDF8F0" : "#9CA3AF",
                  fontWeight: 600,
                  fontSize: 14,
                ...
                onMouseOver={(e) => {
                  if (canGoNext()) e.currentTarget.style.background = "#5B2C74"
                }}
                onMouseOut={(e) => {
                  if (canGoNext()) e.currentTarget.style.background = "#3D1550"
                }}
              >
                Continue
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 14,
                  cursor:
                    data.confirmed && !submitting ? "pointer" : "not-allowed",
                  minHeight: 46,
                  fontFamily: "Manrope, sans-serif",
                }}
              >
